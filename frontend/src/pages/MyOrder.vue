<template>
    <div class="my-order-container" :class="filterBills.length > 0 ? '' : 'fit-screen'">
        <div v-if="filterBills.length > 0" class="my-order-cards">
            <div v-for="b in filterBills.slice().reverse()" class="card" :key="b.bill_id">
                <div class="card-head d-flex flex-wrap flex-sm-nowrap justify-content-between">
                    ...
                    <button @click="sendBillId(b.bill_id)">detail pesanan</button>
                </div>

                <div class="d-flex flex-wrap flex-sm-nowrap justify-content-between card-summary">
                    <div class="w-100 text-center py-1 px-2"><span>Pembayaran:</span>{{ " Belum dibayar"  }}
                    </div>
                    <div class="w-100 text-center py-1 px-2"><span>Status:</span>{{ " " + avaiableStatus[b.bill_status]
                    }}
                    </div>
                    <div class="w-100 text-center py-1 px-2"><span>Waktu:</span> {{ b.bill_when }}</div>
                </div>
                <div class="d-flex flex-wrap flex-sm-nowrap justify-content-between card-summary">

                    <div class="w-100 text-center py-1 px-2"><span>Total:</span> Rp {{ b.bill_total }}.000</div>
                    <div class="w-100 text-center py-1 px-2"><span>Alamat:</span>{{ " " + b.bill_address }}
                    </div>
                    <div class="w-100 text-center py-1 px-2"><span>Telepon:</span>{{ " " + b.bill_phone }}
                    </div>
                </div>
        ...
        <div v-else class="box-content row no-food">
            <div class="content">
                <h2 style="color: #057835fa;">Kamu masih belum memiliki pesanan</h2>
            </div>
            <div>
                <img src="../assets/images/no-orders.png" alt="" />
            </div>
            <router-link class="btn" to="/menu">Pesan sekarang!</router-link>
        </div>
...
export default {
    ...
    data() {
        return {
            avaiableStatus: ["cancel", "diterima", "preparing", "checking", "delivering", "delivered"],
            allBills: [],
