        ...
        <div class="checkout-form-container">
            <form id="checkoutForm" @submit="handleSubmit" novalidate autocomplete="off">
                <div class="checkout-heading">
                    <h3>Beberapa Langkah Lagi untuk Membeli<span>Total</span></h3>
                    <h3 v-if="user">Pesanan {{ user.user_name }}<span>Rp {{ calculateSummaryPrice()[3] }}.000</span></h3>
                </div>

                <div class="form-group details-group">
                    <h4>Detail Kontak</h4>
                    <div class="form-group">
                        <input type="text" name="coPhone" id="coPhone" placeholder="Nomor Telepon" class="form-control"
                ...
                <div class="form-group details-group">
                    ...
                    <div class="form-group">
                        <div class="form-group">
                            ...
                            <input type="radio" name="payment" value="card" id="paymentCard"
                                v-model="checkoutObj.paymentMethod" /><span>Debit</span>
                        </div>
                    ...
                    <div v-if="checkoutObj.paymentMethod == 'card'">
                        <div class="form-group">
                            <input type="text" name="coCardNum" placeholder="Masukkan Nomor Kartu" id="coCardNum"
                        ...
                        <div class="form-group">
                            <input v-upcase type="text" name="coCardName" placeholder="Masukkan Nama Kartu "
                ...
                <div v-if="user" class="form-group">
                    <input type="submit" value="Setuju & Bayar" class="btn"
    ...
    methods: {
        ...
        checkForm: function () {
            ...
            if (!this.checkoutObj.phone) {
                this.errorObj.phoneErr.push('Wajib Isi Nomor');
            }
            else {
                if (!this.checkoutObj.phone.startsWith('62')) {
                    this.errorObj.phoneErr.push('Nomor harus diawali 62');
                }

                if (this.checkoutObj.phone.length != 13) {
                    this.errorObj.phoneErr.push('Nomor Harus 13 Digit');
                }

                if (!/[0-9]{13}/.test(this.checkoutObj.phone)) {
                    this.errorObj.phoneErr.push('Nomor Hanya Berisi Angka');
                }
            }

            // Address validate
            if (!this.checkoutObj.address) {
                this.errorObj.addressErr.push('Wajib Masukkan Alamat');
            }

            // Card validate
            if (!this.checkoutObj.paymentMethod) {
                this.errorObj.payErr.push('Wajib Pilih Metode');
            }
            else if (this.checkoutObj.paymentMethod == "card") {
                if (!this.cardObj.number) {
                    this.errorObj.numErr.push('Wajib Masukkan nomor Kartu');
                }
                else {
                    if (!this.cardObj.number.startsWith('4')) {
                        this.errorObj.numErr.push('Visa diawali angka 4');
                    }

                    if (this.cardObj.number.length != 16) {
                        this.errorObj.numErr.push('Visa Harus berisi 16 digit');
                    }

                    if (!/[0-9]{16}/.test(this.cardObj.number)) {
                        this.errorObj.numErr.push('Visa Hanya Boleh Angka');
                    }
                }

                if (!this.cardObj.name) {
                    this.errorObj.nameErr.push('Wajib Masukkan Nama');
                }
                else {
                    if (!/^[A-Za-z]+$/.test(this.cardObj.name.replace(/\s/g, ""))) {
                        this.errorObj.nameErr.push('Nama Hanya Berisi Huruf');
                    }
                }

                if (!this.cardObj.expiryDate) {
                    this.errorObj.exDateErr.push('Wajib Mengis Kadaluarsa Kartu');
                }


                if (!this.cardObj.cvv) {
                    this.errorObj.cvvErr.push('Wajib Mengisi CVV');
                }
                else {
                    if (this.cardObj.cvv.length != 3) {
                        this.errorObj.cvvErr.push('CVV hanya 3 Digit');
                    }

                    if (!/[0-9]{3}/.test(this.cardObj.cvv)) {
                        this.errorObj.cvvErr.push('CVV hanya boleh berisi Angka');
                    }
                }
